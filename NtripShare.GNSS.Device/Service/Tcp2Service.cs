...
{
	public class Tcp2Service : BaseService
	{
		...
		private static Tcp2Service Instance = null;
		public static Tcp2Service getInstance()
		{
			if (Instance == null)
			{
				Instance = new Tcp2Service();
			}
			return Instance;
		}
					...
					if (bootstrap == null)
					{
						...
						bootstrap
							...
							.Handler(new ActionChannelInitializer<ISocketChannel>(channel =>
							{
								...
								pipeline.AddLast("TcpServiceHandler2", new TcpServiceHandler());
							}));
					}
					while (IsServiceStarted && !IsConnected)
					{
						TcpConfig tcpConfig = SysConfig.getInstance().TcpConfig2;
						...
						//bootstrap.ConnectAsync(tcpConfig.TcpIP, tcpConfig.TcpPort);
						log.Info($"TcpServiceHandler2.ConnectAsync-{IsConnected}--{tcpConfig.TcpIP}:{tcpConfig.TcpPort}");
						Thread.Sleep(5000);
					}
				}
				catch (Exception ex)
		...
		private class TcpServiceHandler : FlowControlHandler
		{
			...
			{
				IChannel channel = Tcp2Service.getInstance().ServerChannel;
				Tcp2Service.getInstance().IsConnected = true;
				Tcp2Service.getInstance().ServerChannel = context.Channel;
                if (!Tcp2Service.getInstance().IsServiceStarted)
                {
				...
				Tcp2Service.getInstance().ICY200OK = true;
				log.Info("Tcp2Service平台已连接");
			}
			...
			{
				if (Tcp2Service.getInstance().ServerChannel != null && context.Channel.Id != Tcp2Service.getInstance().ServerChannel.Id)
				{
					return;
				}
				Tcp2Service.getInstance().IsConnected = false;
				Tcp2Service.getInstance().ICY200OK = false;
				Tcp2Service.getInstance().ServerChannel = null;
				base.ChannelInactive(context);
				log.Info("Tcp2Service平台已duan连接");
				SysConfig mosConfig = SysConfig.getInstance();
				if (!mosConfig.TcpConfig2.TcpEnable)
                {
                    return;
                }
				...
				timer.Elapsed += new System.Timers.ElapsedEventHandler((s, x) =>
				{
					...
					if ( mosConfig.TcpConfig2.TcpEnable)
					{
						...
						if (StringHelper.IPCheck(tcpConfig.TcpIP))
						{
							Tcp2Service.getInstance().bootstrap.ConnectAsync(new IPEndPoint(IPAddress.Parse(tcpConfig.TcpIP), tcpConfig.TcpPort));
						}
						else
						{
							Tcp2Service.getInstance().bootstrap.ConnectAsync(tcpConfig.TcpIP, tcpConfig.TcpPort);
						}
						//Tcp2Service.getInstance().bootstrap.ConnectAsync(tcpConfig.TcpIP, tcpConfig.TcpPort);
					}
				});
